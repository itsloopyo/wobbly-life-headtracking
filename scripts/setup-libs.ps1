#!/usr/bin/env pwsh
# Provisions the BepInEx reference assemblies into src/WobblyLifeHeadTracking/libs/
# from the committed vendor zip. The Unity reference stubs are built separately by
# cameraunlock-core/csharp/stubs/build-unity-stubs.ps1, which `pixi run setup` runs
# after this. No Wobbly Life installation needed.
...
$libsPath     = Join-Path $projectRoot 'src\WobblyLifeHeadTracking\libs'
...
$tempDir = Join-Path $env:TEMP ("wl-bep-" + [Guid]::NewGuid().ToString('N'))
